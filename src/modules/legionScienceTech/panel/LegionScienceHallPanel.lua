--region LegionScienceHallPanel.lua
--Author : admin
--Date   : 2017/8/28
--此文件由[BabeLua]插件自动生成
...
LegionScienceHallPanel = class("LegionScienceHallPanel", BasicPanel)
LegionScienceHallPanel.NAME = "LegionScienceHallPanel"
LegionScienceHallPanel.CONTRIBUTE_GOLD_TYPE = 1 -- 元宝类型
LegionScienceHallPanel.CONTRIBUTE_RES_TYPE = 2  -- 资源类型

local MAXLEVEL = 30 --最高等级

function LegionScienceHallPanel:ctor(view, panelName)
    LegionScienceHallPanel.super.ctor(self, view, panelName)
    
    self:setUseNewPanelBg(true)
end

function LegionScienceHallPanel:finalize()
    LegionScienceHallPanel.super.finalize(self)
end

function LegionScienceHallPanel:initPanel()
	LegionScienceHallPanel.super.initPanel(self)
...
function LegionScienceHallPanel:doLayout()
	--local topAdaptivePanel = self:topAdaptivePanel()
    --local tabsPanel = self:getTabsPanel()
	--NodeUtils:adaptiveUpPanel(self._topPanel, tabsPanel, GlobalConfig.tabsHeight * 0.5)
...
function LegionScienceHallPanel:registerEvents()
	LegionScienceHallPanel.super.registerEvents(self)
end

-- 关闭
function LegionScienceHallPanel:onHideHandler()
    --print("执行了 onClosePanelHandler")
    -- 根据配置表判断
...
function LegionScienceHallPanel:getHallListData()
	-- body
	local tabData = {}
...
function LegionScienceHallPanel:onAfterActionHandler()
    self:onHallInfoResp(self._lastHallUpgrateData)
end

function LegionScienceHallPanel:onShowHandler()
...
-- 军团大厅220007更新
function LegionScienceHallPanel:onHallInfoResp( data )
...
-- 军团大厅初始化or更新
function LegionScienceHallPanel:onHallUpdate(data)
...
-- 大厅捐献回调，确认贡献成功
function LegionScienceHallPanel:onHallContributeResp(data)
...
function LegionScienceHallPanel:onTopPanel()
	-- body
...
function LegionScienceHallPanel:onHallList()
	-- body
...
--刷新单个数据
function LegionScienceHallPanel:updateRoleInfoHandler()
	local legionProxy = self:getProxy(GameProxys.Legion)
	self._resCurNumber = legionProxy:getResNumber()
	self._curCount = legionProxy:getHallCurCount()
	local tabData = legionProxy:getResData(self._curCount)
...
function LegionScienceHallPanel:onRenderItemAll(tabData)
...
function LegionScienceHallPanel:onRenderItemFirst(itempanel, info, index)
...
function LegionScienceHallPanel:onRenderItem(itempanel, info, index)
	-- body
...
-- 大厅升级按钮
function LegionScienceHallPanel:onTopUpBtn(sender)
	-- body
	-- print("top up btn")
...
function LegionScienceHallPanel:onUpgrateBtn(sender)
    	...
    	if restype == 200 then
    		...
    		local function okCallBack()
    	    	...
    	    	local function callFunc()
                    ...
                    -- self._legionProxy:addContributeInfo(LegionScienceHallPanel.CONTRIBUTE_GOLD_TYPE, resNeedCount) -- 1元宝
    	    	    self._reqContributeType = LegionScienceHallPanel.CONTRIBUTE_GOLD_TYPE
                end
			...
			local legionProxy = self:getProxy(GameProxys.Legion)
	    	legionProxy:onTriggerNet220008Req(data)
            -- self._legionProxy:addContributeInfo(LegionScienceHallPanel.CONTRIBUTE_RES_TYPE, resNeedCount) -- 2资源
    	    self._reqContributeType = LegionScienceHallPanel.CONTRIBUTE_RES_TYPE
        end
...
-- 按钮灰色，达到次数上限
function LegionScienceHallPanel:onUpNotChance()
    self:showSysMessage(self:getTextWord(3158)) -- [[您的捐献次数已达上限了]]
end


-- 是否弹窗元宝不足
function LegionScienceHallPanel:isShowRechargeUI(sender)
    -- body
...
function LegionScienceHallPanel:onClosePanelHandler()
    self:onHideHandler()
end
