...
UIRankPanel = class("UIRankPanel", BasicComponent) 

function UIRankPanel:ctor(parent, panel, closeFunction, data)
	UIRankPanel.super.ctor(self)
    local uiSkin = UISkin.new("UIRankPanel")
    uiSkin:setParent(parent)
    self._uiSkin = uiSkin
    self._parent = parent
    self._panel = panel

    self._uiPanelBg = UIPanelBgNew.new(self._uiSkin:getRootNode(), function()
    ...
    self._uiPanelBg:setBgType(ModulePanelBgType.NONE)
...
function UIRankPanel:initPanel(info)
...
function UIRankPanel:finalize()
	self._uiSkin:finalize()
	self._uiSkin = nil
	UIRankPanel.super.finalize(self)
end

function UIRankPanel:setTitle(isShow,content, isImg, exContent)
    self._uiPanelBg:setIsShowName(isShow,content, isImg, exContent)
end

function UIRankPanel:updateData(info)  --刷新listview数据
	...
	info.titleText = info.titleText or "积分"
	...
	if info.tipText ~= nil then
		self._panel:addTouchEventListener(self.tip_btn, function()
			local uiTip = UITip.new(self._parent)
    ...
    local posY = self:getChildByName("Panel_14/top_panel/Image_13"):getPositionY()
...
function UIRankPanel:renderItem(item, data, index)
...
function UIRankPanel:getChildByName(name)
    return self._uiSkin:getChildByName(name)
end

function UIRankPanel:renderRewardItem(item, data, index)
...
function UIRankPanel:setMask()--设置灰色屏幕
