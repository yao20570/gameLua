...
--  * @Description: 
--  */
WorldHelpPanel = class("WorldHelpPanel", BasicPanel)
WorldHelpPanel.NAME = "WorldHelpPanel"

function WorldHelpPanel:ctor(view, panelName)
    local layer = view:getLayer(ModuleLayer.UI_TOP_LAYER)
    WorldHelpPanel.super.ctor(self, view, panelName, 800, layer)
    
    self:setUseNewPanelBg(true)
end

function WorldHelpPanel:finalize()
    if self._effect ~= nil then
        self._effect:finalize()
        self._effect = nil
    end

    WorldHelpPanel.super.finalize(self)
end

function WorldHelpPanel:initPanel()
	WorldHelpPanel.super.initPanel(self)
...
function WorldHelpPanel:onShowHandler()
    self:topBtnTouch({id = 1})
end

function WorldHelpPanel:topBtnTouch(sender)
...
function WorldHelpPanel:updateView(index)
    for i=1,3 do
...
function WorldHelpPanel:changEffect(curPauseIndex, index)
    local effectName = {{name = "rgb-wfsm-gongjidiren"}, {name = "rgb-wfsm-caiji"}, {name = "rgb-wfsm-yuanzhu"}}
...
function WorldHelpPanel:pauseFunc(pauseIndex, index)
    local maxCounts = {3, 4, 2}
...
function WorldHelpPanel:hide()
    if self._effect ~= nil then
        self._effect:finalize()
        self._effect = nil
    end
    WorldHelpPanel.super.hide(self)
end

-- //null 初始化新的panel 老版依旧保留
function WorldHelpPanel:initPanelNew()
--	logger:info("初始化新的界面")
...
function WorldHelpPanel:dirBtnTouch(sender)
...
function WorldHelpPanel:initDataPanel()
...
--//null 刷新当前选中界面内容
function WorldHelpPanel:updateGroup(id,group)
    group = group or 1
    id = id or 1
...
function WorldHelpPanel:initEvent()
...
function WorldHelpPanel:updateCheckBox(sender)
...
function WorldHelpPanel:updateIcon(group,YMax)
