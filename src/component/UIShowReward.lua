...
UIShowReward = class("UIShowReward")

function UIShowReward:ctor(parent, panel,isShow,callfunc)
    local uiSkin = UISkin.new("UIShowReward")
    ...
    local secLvBg = UISecLvPanelBg.new(layout, self) -- 新版的底图框
...
function UIShowReward:setTitle(titileStr)
    self.secLvBg:setTitle(titileStr)
end

function UIShowReward:hide()
...
function UIShowReward:getChildByName(name)
    return self._uiSkin:getChildByName(name)
end

function UIShowReward:playAction(callback, isHideEff)    
    -- 半透明背景坐标偏移的微调
...
function UIShowReward:show(info, callback, isHideEff)
    -- body
...
function UIShowReward:renderAllGoods(infos)
    self._uiSkin:setVisible(true)
    local scale = NodeUtils:getAdaptiveScale()
    local listView = self:getChildByName("mainPanel/ListView_1")

    ComponentUtils:renderAllGoods(listView, self.bgImg, self.secLvBg, self.sureBtn, infos, self._panel)
    --local size = self.secLvBg:getContentSize()
    --self.bgImg:setContentSize(size.width, size.height - 120)
    -- local closeBtn = self:getChildByName("mainPanel/sureBtn")self.secLvBg
    ComponentUtils:addTouchEventListener(self.sureBtn, self.hide, nil, self)
end

-- 特效
function UIShowReward:createEffect()
    -- body
...
-- 加载获得物品的背景特效
function UIShowReward:playEffect()
...
function UIShowReward:infoTodouble(info)
...
function UIShowReward:scrollToEnd()
    self._listView:scrollToBottom(2.5,true)
end

function UIShowReward:renderItemPanel(item, data)
...
function UIShowReward:adjustIconPos(item, lenght)
...
function UIShowReward:removeFromParent()
    if self.movieChip ~= nil then
...
--外部 设置==================================
function UIShowReward:setBtnState( strTitleText, isGray )
...
function UIShowReward:setBtnCallback( callback )
...
function UIShowReward:finalize()
