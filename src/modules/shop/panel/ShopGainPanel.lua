...
ShopGainPanel = class("ShopGainPanel", BasicPanel)
ShopGainPanel.NAME = "ShopGainPanel"

function ShopGainPanel:ctor(view, panelName)
    ShopGainPanel.super.ctor(self, view, panelName)
    
    self:setUseNewPanelBg(true)
end

function ShopGainPanel:finalize()
    ShopGainPanel.super.finalize(self)
end

function ShopGainPanel:initPanel()
    ShopGainPanel.super.initPanel(self)
    self:initListView()
end

function ShopGainPanel:doLayout()
    ...
    NodeUtils:adaptiveListView(self._scrollView, 
                        GlobalConfig.downHeight, tabsPanel,GlobalConfig.topTabsHeight)
    self:createScrollViewItemUIForDoLayout(self._scrollView)
end

--显示界面时调用
function ShopGainPanel:onShowHandler(data)
...
--初始化listVIew
function ShopGainPanel:initListView()
...
-- function ShopGainPanel:updateListView(data)
...
function ShopGainPanel:updateScrollView(data)
    local shopPanel = self:getPanel(ShopPanel.NAME)
    local data = shopPanel:getShopConfigDataByType(2)
...
function ShopGainPanel:renderItemPanel(itemPanel,info,idx)
...
--[[
function ShopGainPanel:renderItemPanel(itemPanel,info)
...
--获取item的子节点
-- function ShopGainPanel:getItemChildren(item)
--     local itemChildren = {}
...
function ShopGainPanel:initItem(item)
...
--@data:数组 key = 1...n
--@return:{} --二维数组
function ShopGainPanel:translateDataFormat(data)
...
-----------回调函数定义----------------
--购买按钮回调
function ShopGainPanel:onBuyBtnTouch(sender)
